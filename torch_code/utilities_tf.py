...
def load_batch_gcnn(sample_files, device):
    ...
    e_indices = torch.tensor(e_indices, dtype=torch.long).to(device)
    ...
    n_cs_per_sample = torch.tensor(n_cs_per_sample, dtype=torch.int32).to(device)
    n_vs_per_sample = torch.tensor(n_vs_per_sample, dtype=torch.int32).to(device)
